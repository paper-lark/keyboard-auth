...
describe('KeyLogger', () => {
  ...
  it('should call subscribe to IO events', () => {
    ...
    expect(mockIO.on.mock.calls.map(e => e[0]).sort()).toEqual(
      ['keyup', 'keydown'].sort()
    );
  });
  ...
  it('should call callbacks when key is pressed and released', () => {
    ...
    // emit key events
    const keyDownHandlers = mockIO.on.mock.calls
      .filter(e => e[0] === 'keydown')
      .map(e => e[1]);
    const keyUpHandlers = mockIO.on.mock.calls
      .filter(e => e[0] === 'keyup')
      .map(e => e[1]);
